import { type File as FormidableFile, IncomingForm } from 'formidable'
    ...
    form.parse(req, async (err, fields, files: Files) => {
      ...
      if (!dateEntry) {
        return res.status(422).json({
          ...
          message:
            'Para registrar um registro, é necessário informar a data de registro',
        })
      }
      ...
      if (!image || (Array.isArray(image) && image.length === 0)) {
        return res.status(400).json({
          ...
          message:
            'Para registrar um registro, é necessário informar uma imagem',
        })
      }

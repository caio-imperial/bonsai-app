...
const EntriesHeaderMenuActions = ({ bonsai, entriesLoading, handleFavorite, handleEdit, handleAddEntry }: EntriesHeaderMenuActionsProps) => {
    return (
        <div className="flex gap-2">
            ...
            <Button variant="secondary" disabled={entriesLoading || !bonsai} onClick={(e) => { handleEdit({ bonsaiId: bonsai?._id || '' }); e.stopPropagation() }} className="cursor-pointer">
                <PencilIcon className="w-4 h-4" /> Editar
            </Button>
            <Button variant="secondary" disabled={entriesLoading || !bonsai} onClick={(e) => { handleAddEntry({ bonsaiId: bonsai?._id || '' }); e.stopPropagation() }} className="cursor-pointer">
